...
import { makeTempColor, rampRange, chrome, fmtAnomaly } from '../lib/climateScale';
      ...
      >
        <defs>
          <linearGradient id="trend-grad" gradientUnits="userSpaceOnUse" x1={0} y1={y(1.5)} x2={0} y2={y(-0.8)}>
            <stop offset="0%" stopColor={ramp[5]} />
            <stop offset={`${((1.5 - 1.0) / 2.3) * 100}%`} stopColor={ramp[4]} />
            <stop offset={`${((1.5 - 0.5) / 2.3) * 100}%`} stopColor={ramp[3]} />
            <stop offset={`${((1.5 - 0.0) / 2.3) * 100}%`} stopColor={ramp[2]} />
            <stop offset={`${((1.5 + 0.3) / 2.3) * 100}%`} stopColor={ramp[1]} />
            <stop offset="100%" stopColor={ramp[0]} />
          </linearGradient>
        </defs>
        ...
        {Y_TICKS.map((t) => (
          <g key={t}>
            <line
              ...
              y1={y(t)}
              y2={y(t)}
              stroke={t === 0 ? c.ink(0.35) : c.ink(0.1)}
        ...
        {[1.0, 1.5].map((t) => (
          <text key={t} x={M.left + IW + 6} y={y(t)} dy="0.32em" fontSize={10} fontFamily="var(--font-mono)" fill={c.ink(0.45)}>
            {`+${t.toFixed(1)}`}
          </text>
        ))}
        ...
        <path d={geom.lineD} fill="none" stroke={c.ink(0.7)} strokeWidth={1.5} strokeLinejoin="round" />

        {/* x axis */}
        {X_TICKS.map((t) => (
          <text key={t} x={x(t)} y={VBH - 10} textAnchor="middle" fontSize={11} fontFamily="var(--font-mono)" fill={c.ink(0.5)}>
            {t}
          </text>
        ))}
        ...
        {hovered && (
          <g>
            <line x1={x(hovered.year)} x2={x(hovered.year)} y1={M.top} y2={M.top + IH} stroke={c.ink(0.35)} strokeWidth={1} />
            <circle cx={x(hovered.year)} cy={y(hovered.v)} r={4} fill={tempColor(hovered.v)} stroke={c.surface} strokeWidth={1.5} />
          </g>
        )}
      </svg>
    </div>
